...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  //INSTPAT("??????? ????? ????? 000 ????? 00101 11", seqz, R, R(rd) = src1 == 0 ? 1 : 0);
  //----------------------------------R---------------------------------------------
  ...
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, s -> dnpc += (sword_t)src1 >= (sword_t)src2 ? imm - 4 : 0);
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh  , R, R(rd) = ((sword_t)src1 * (sword_t)src2) >> (sizeof(sword_t) * 4));
